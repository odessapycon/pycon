...
export class RenderSpeakers2020 {
  constructor(options) {
  ...
  _init() {
    ...
    let speakerItem =
      ...
      '<div class="speaker-img-2020">' +
      '<img data-src="${image}" alt="${name}">' +
      '</div>' +
      '<div class="speaker-name speaker-name-2020">${name}</div>' +
      '<div class="speaker-position-2020">${position} ' +
      '{{if company}}' +
      '@${company}' +
      '{{/if}}' +
      '</div>' +
      '<div class="speaker-place-2020">${place}</div>' +
      '<div class="speaker-report-2020">' +
      '{{each rept }} {{html $value.title}} {{if $value.title}}</br> </br>{{/if}}{{/each}}' +
      '</div>' +
      '</div>' +
      '{{if socials}}<div class="speaker-socials-2020">{{html socialsRendered}}</div>{{/if}}' +
      '{{if slideshare}}'+
    ...
    var socialsItem = '<a href=\'${link}\' class=\'${fatype =="github" ? "github" : ""}\' target=\'_blank\'><i class=\'fa fa-${fatype}\' aria-hidden=\'true\'></i></a>';
